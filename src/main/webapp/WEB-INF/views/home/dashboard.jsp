...
<body>
        ...
        <div class="main-area">
            ...
            <section class="today-section">
                <div class="section-header">
                    <h2 class="section-title">📅 오늘 할 일</h2>
            ...
            <!-- 기타 할 일 섹션 -->
            <section class="other-section dropzone" data-period="OTHER">
                <h2 class="section-title">📋 기타 할 일</h2>
                
                <!-- TaskList별로 그룹화해서 표시 -->
                <c:set var="currentTaskListName" value=""/>
                <c:set var="hasAnyTasks" value="false"/>
                
                <c:forEach var="task" items="${notTodayTasks}" varStatus="status">
                    <c:set var="hasAnyTasks" value="true"/>
                    
                    <!-- 새로운 TaskList 시작 시 헤더 표시 -->
                    <c:if test="${task.taskListName != currentTaskListName}">
                        <c:set var="currentTaskListName" value="${task.taskListName}"/>
                        <div class="task-list-group">
                            <div class="task-list-header">📁 ${task.taskListName}</div>
                    </c:if>
                    ...
                    <div class="task-card" 
                        ...
                        <div class="task-meta">
                            ...
                            <c:if test="${not empty task.deadline}">
                                <c:if test="${not empty task.estimateMin}"> • </c:if>
                                📅 ${fn:substring(task.deadline, 0, 10)}
                            </c:if>
                        </div>
                    </div>
                    
                    <!-- 다음 Task가 다른 TaskList이거나 마지막 Task인 경우 그룹 닫기 -->
                    <c:set var="nextTaskListName" value=""/>
                    <c:if test="${status.index + 1 < fn:length(notTodayTasks)}">
                        <c:set var="nextTaskListName" value="${notTodayTasks[status.index + 1].taskListName}"/>
                    </c:if>
                    
                    <c:if test="${nextTaskListName != currentTaskListName or status.last}">
                        </div> <!-- task-list-group 닫기 -->
                    </c:if>
                </c:forEach>
                
                <c:if test="${!hasAnyTasks}">
                    <div class="empty-state">기타 할 일이 없습니다</div>
                </c:if>
            </section>
        </div>
    </div>
    ...
    <script>
        ...
        function showCreateTaskModal() {
            ...
            document.getElementById('periodDayGroup').style.display = 'none';
        ...
        async function toggleTask(taskId) {
            console.log('toggleTask called with taskId:', taskId, 'type:', typeof taskId);
        ...
        async function deleteTaskDirectly(taskId) {
            console.log('deleteTaskDirectly called with taskId:', taskId, 'type:', typeof taskId);
        ...
        async function handleDrop(e) {
            ...
            try {
                // 기타 할 일(OTHER)로의 이동
                if (targetPeriod === 'OTHER') {
                    const response = await fetch('/tasks/' + draggedTaskId + '/move-to-other', {
                        method: 'PUT',
                        headers: getCSRFHeaders()
                    });
